  ...
  for (let antennaTypeIndex = 0; antennaTypeIndex < Object.keys(antennaLocations).length; antennaTypeIndex++) {
    ...
    for (let i1 = 0; i1 < locations.length; i1++) {
      for (let i2 = 0; i2 < locations.length; i2++) {
        ...
        // first - diff = second
        const firstAntiNode = {
          x: firstAntenna.x + locationDifference.x,
          y: firstAntenna.y + locationDifference.y,
        };
        const secondAntiNode = {
          x: secondAntenna.x - locationDifference.x,
          y: secondAntenna.y - locationDifference.y,
        };

        if (!antiNodeLocations.some(l => l.x === firstAntiNode.x && l.y === firstAntiNode.y)) {
          antiNodeLocations.push(firstAntiNode);
        }

        if (!antiNodeLocations.some(l => l.x === secondAntiNode.x && l.y === secondAntiNode.y)) {
          antiNodeLocations.push(secondAntiNode);
        }
      }
    }
  }
